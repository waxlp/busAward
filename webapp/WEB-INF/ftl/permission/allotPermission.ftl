...
<script>
			...
			*/
			function selectPermissionById(id){
				var load = layer.load();
				$.post("${basePath}/permission/selectPermissionById.shtml",{roleId:id},function(result){
					layer.close(load);
...
<body data-target="#one" data-spy="scroll">
	...
	<div class="container"
				...
				<form method="post" action="" id="formId" class="form-inline">
					<div clss="well">
						...
						<span class=""> <#--pull-right -->
							...
							<@shiro.hasPermission
							name="/permission/clearPermissionByRoleIds.shtml">
					...
					<table class="table table-bordered">
						...
						<tr>
							<td><input value="${it.roleId}" check='box' type="checkbox" /></td>
							<td>${it.roleName}</td>
							<td permissionIds="${it.permissionIds?default('')}">${it.permissionNames?default('-')}</td>
							<td><@shiro.hasPermission
								name="/permission/addPermission2Role.shtml"> <i
								class="glyphicon glyphicon-share-alt"></i><a
								href="javascript:selectPermissionById(${it.roleId});">选择权限</a>
								</@shiro.hasPermission>
							</td>
						</tr>
						</#list> <#else>
		...
		<#--/row--> <#--弹框-->
		<div class="modal fade bs-example-modal-sm" id="selectPermission"
			tabindex="-1" role="dialog" aria-labelledby="selectPermissionLabel">
			<div class="modal-dialog modal-sm" role="document">
				<div class="modal-content">
					...
					<div class="modal-footer">
						...
						<button type="button" onclick="selectPermission();"
