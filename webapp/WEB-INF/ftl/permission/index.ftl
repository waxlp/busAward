...
<script>
			...
			function addPermission(){
				...
				<#--loding-->
				var load = layer.load();
				$.post('${basePath}/permission/addPermission.shtml',{permissionName:name,url:url},function(result){
					layer.close(load);
					if(result && result.status != 200){
...
<body data-target="#one" data-spy="scroll">
	...
	<div class="container"
			...
			<div class="col-md-10">
				...
				<form method="post" action="" id="formId" class="form-inline">
					<div clss="well">
						<div class="form-group">
							<input type="text" class="form-control" style="width: 300px;"
								value="${findContent?default('')}" name="findContent"
								id="findContent" placeholder="输入权限名称">
						</div>
						<span class=""> <#--pull-right -->
							<button type="submit" class="btn btn-primary">查询</button>
							<@shiro.hasPermission name="/permission/addPermission.shtml"> <a
							class="btn btn-success" onclick="$('#addPermission').modal();">增加权限</a>
							</@shiro.hasPermission> <@shiro.hasPermission
							name="/permission/deletePermissionById.shtml">
					...
					<table class="table table-bordered">
						...
						<tr>
							<td><input value="${it.permissionId}" check='box'
								type="checkbox" /></td>
							<td>${it.permissionName?default('-')}</td>
							<td>${it.description?default('-')}</td>
							<td>
							<@shiro.hasPermission name="/permission/deletePermissionById.shtml"> <i
								class="glyphicon glyphicon-remove"></i><a
								href="javascript:deleteById([${it.permissionId}]);">删除</a>
							</@shiro.hasPermission>
							</td>
						</tr>
						</#list> <#else>
		...
		name="/permission/addPermission.shtml"> <#--弹框-->
