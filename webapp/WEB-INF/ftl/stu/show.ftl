...
<head>
	<meta charset="utf-8" />
	<title>${stu.realName?default('未录入')} —个人中心</title>
			...
			<div class="row">
				...
				<div class="col-md-10">
					...
					<table class="table table-bordered">
						...
						<tr>
							<th>姓名</th>
							<td>${stu.realName?default('未录入')}</td>
						</tr>
						<tr>
							<th>家庭住址</th>
							<td>${stu.stuAddress?default('未录入')}</td>
						</tr>
						<tr>
							<th>出生日期</th>
							<td>${stu.stuBirthday?string('yyyy-MM-dd')}</td>
						</tr>
						<tr>
							<th>上次登录时间</th>
			  ...
			  <div class="modal-body">
			    <form class="form-horizontal">
				  ...
				  <div class="control-group">
				    ...
				    <div class="controls">
				      <input type="text" id="updateAnswer" class="form-control"placeholder="答案">
				    </div>
				  </div>
	...
	<script>
		function addIssue(){
			var quesstion1 = $("#addQuesstion1").val(),
				addAnswer1 = $("#addAnswer1").val(),
				quesstion2 = $("#addQuesstion2").val(),
				addAnswer2 = $("#addAnswer2").val();
			if($.trim(addAnswer1) == ''|| $.trim(addAnswer2) == ''){
					return layer.msg('答案不能为空。',so.default),!1;
				}	
		}
