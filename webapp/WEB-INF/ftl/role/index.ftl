...
<script>
			...
			function addRole(){
				...
				<#--loding-->
				var load = layer.load();
				$.post('${basePath}/role/addRole.shtml',{roleName:name},function(result){
					layer.close(load);
					if(result && result.status != 200){
...
<body data-target="#one" data-spy="scroll">
	...
	<div class="container" style="padding-bottom: 15px; min-height: 300px; margin-top: 40px;">
					...
					<div clss="well">
						<div class="form-group">
							<input type="text" class="form-control" style="width: 300px;"
								...
								id="findContent" placeholder="输入角色类型 / 角色名称">
						</div>
						<span class=""> <#--pull-right -->
							<button type="submit" class="btn btn-primary">查询</button>
							<@shiro.hasPermission name="/role/addRole.shtml"> <a
							class="btn btn-success" onclick="$('#addrole').modal();">增加角色</a>
							</@shiro.hasPermission> <@shiro.hasPermission
							name="/role/deleteRoleById.shtml">
		...
		<#--/row--> <@shiro.hasPermission name="/role/addRole.shtml">
		<#--添加弹框-->
