...
<script>
		so.init(function(){
			...
			function selectRoleById(id){
				var load = layer.load();
				$.post("${basePath}/role/selectRoleByUserId.shtml",{userId:id},function(result){
					layer.close(load);
...
<body data-target="#one" data-spy="scroll">
	...
	<div class="container"
			...
			<div class="col-md-10">
				...
				<form method="post" action="" id="formId" class="form-inline">
					...
					<table class="table table-bordered">
						...
						<tr>
							<td><input value="${it.userId}" check='box' type="checkbox" /></td>
							<td>${it.userName}</td>
							<td>${(it.satus==1)?string('有效','禁止')}</td>
							<td roleIds="${it.roleIds?default('')}">${it.roleNames?default('-')}</td>
							<td><@shiro.hasPermission name="/role/addRole2User.shtml"> <i
								class="glyphicon glyphicon-share-alt"></i>
								<a href="javascript:selectRoleById(${it.userId});">选择角色</a>
								</@shiro.hasPermission>
							</td>
						</tr>
						</#list> <#else>
		...
		<#--/row--> <#--弹框-->
		<div class="modal fade bs-example-modal-sm" id="selectRole"
