package com.zz80z.busAward.core.shiro.cache;
...
import com.zz80z.busAward.common.utils.SerializeUtil;
import com.zz80z.busAward.core.shiro.session.CustomSessionManager;
import com.zz80z.busAward.core.shiro.session.SessionStatus;
...
public class JedisShiroSessionRepository implements ShiroSessionRepository {
	...
	public void saveSession(Session session) {
		...
		try {
			...
			/**
			 * 这里是个严重问题，但是也不会是致命， 我计算了下，之前上面不小心给我加了0，也就是 18000 / 3600 = 5 个小时
			 ...
			 *
			 * 这里没有走【shiro-config.properties】配置文件，需要注意的是【spring-shiro.xml】
			 * 也是直接配置的值，没有走【shiro-config.properties】
			 *
			 * PS : 注意： 这里我们配置 redis的TTL单位是秒，而【spring-shiro.xml】配置的是需要加3个0（毫秒）。 long
			 * sessionTimeOut = session.getTimeout() / 1000; Long expireTime =
			 * sessionTimeOut + SESSION_VAL_TIME_SPAN + (5 * 60);
			 */
